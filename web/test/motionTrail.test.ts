...
describe("trailWindowStartYears", () => {
  it("is always <= currentTimeYears (the trail's oldest end never comes after its newest end)", () => {
    for (const t of [0, 1, -1, 100, -100, 60_000, -60_000, 500_000, -500_000, 1_000_000, -1_000_000]) {
      expect(trailWindowStartYears(t)).toBeLessThanOrEqual(t);
    }
  });

  it("grows the window from zero length right at Today, in the forward direction", () => {
    expect(trailWindowStartYears(0)).toBe(0);
    expect(trailWindowStartYears(100)).toBe(0);
    expect(trailWindowStartYears(TRAIL_WINDOW_YEARS)).toBe(0);
  });

  it("grows the window from zero length right at Today, in the backward direction (symmetric)", () => {
    // At t=-100 the window has only grown to length 100 (=|t|) so far, so
    // its start is -200 (100 years further back than the current -100),
    // not the fixed-length -100 - TRAIL_WINDOW_YEARS.
    expect(trailWindowStartYears(-100)).toBe(-200);
    expect(trailWindowStartYears(-TRAIL_WINDOW_YEARS)).toBe(-2 * TRAIL_WINDOW_YEARS);
  });

  it("holds at the fixed TRAIL_WINDOW_YEARS length once |t| exceeds the window", () => {
    expect(trailWindowStartYears(400_000)).toBe(400_000 - TRAIL_WINDOW_YEARS);
    expect(trailWindowStartYears(-400_000)).toBe(-400_000 - TRAIL_WINDOW_YEARS);
  });

  it("clamps to Epic #238's settled +/-1,000,000-year range even when currentTimeYears is near the boundary", () => {
    expect(trailWindowStartYears(PLAYER_TIME_RANGE_YEARS)).toBe(
      PLAYER_TIME_RANGE_YEARS - TRAIL_WINDOW_YEARS,
    );
    // Near the far (negative) boundary, subtracting the window would
    // otherwise land below -1,000,000.
    const nearBoundary = -PLAYER_TIME_RANGE_YEARS + 100;
    expect(trailWindowStartYears(nearBoundary)).toBeGreaterThanOrEqual(-PLAYER_TIME_RANGE_YEARS);
  });
});

describe("trailSampleTimesYears", () => {
  it("returns an empty array exactly at Today", () => {
    expect(trailSampleTimesYears(0)).toEqual([]);
  });

  it("returns segmentCount + 1 evenly spaced samples from the window start to currentTimeYears", () => {
    const times = trailSampleTimesYears(400_000, TRAIL_WINDOW_YEARS, TRAIL_SEGMENT_COUNT);
...
describe("starTrailPositionsPc", () => {
  it("returns an empty array at Today, matching trailSampleTimesYears", () => {
    ...
    expect(starTrailPositionsPc([0, 0, 0], velocity, 0)).toEqual([]);
  });
  ...
  it("the last position is always exactly the star's current animated position (via the same starPositionAtTime the marker uses)", () => {
    ...
    const positions = starTrailPositionsPc(position, velocity, tYears);
  ...
  it("recomputes fully from the star's real position/velocity for ANY currentTimeYears - correct on a discontinuous scrub jump, not dependent on having visited intermediate frames", () => {
    ...
    const scrubbed = starTrailPositionsPc(position, velocity, 700_000);
  ...
  it("direction reversal: the window continuously re-centers on the current time, so it correctly re-orients as playback direction flips", () => {
    ...
    const forward = starTrailPositionsPc(position, velocity, 400_000);
    // Reversed a bit (time now decreasing).
    const reversed = starTrailPositionsPc(position, velocity, 390_000);
    // Both trails' oldest end should have shifted down by exactly the same
    // 10,000-year step as their newest end - the whole window slides
    // continuously with currentTimeYears, never "stuck" showing stale
    // forward-direction content.
    expect(reversed[reversed.length - 1][0]).toBeLessThan(forward[forward.length - 1][0]);
    expect(reversed[0][0]).toBeLessThan(forward[0][0]);
  });
});
